...
let package = Package(
  ...
  targets: [
    ...
    .target(
      ...
      // NOTE: upstream had `.unsafeFlags(["-Xlinker", "-all_load"])` here, which
      // SwiftPM forbids when the package is a dependency of an app target. The
      // -all_load flag is instead set on the app target's OTHER_LDFLAGS in
      // project.yml, which is the allowed place for it.
    ),
