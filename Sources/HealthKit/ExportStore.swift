...
final class ExportStore: ObservableObject {
    ...
    @discardableResult
    func saveWorkout(markdown: String, digest: String, workoutCount: Int, contents: String, period: String, mode: ExportMode, createdAt: Date) -> ExportRecord {
        let id = UUID()
        let fileName = "\(id.uuidString).md"
        ...
        let record = ExportRecord(
            ...
            dataPoints: workoutCount, sectionCount: 0, fileName: fileName, kind: .workout, contents: contents, period: period)
        records.insert(record, at: 0)
        persistIndex()
        return record
    }
